...
SIGNAL (SIG_OUTPUT_COMPARE1A)          
{
  ...
  /*  EVENT MANGER INVOCATION*/
   if (msec_FLAG == RAISE)    sweepTable(_10_mS_EVENT_Table, MAX_10mS_EVENT);

   if (sec_FLAG  == RAISE)    sweepTable(SEC_EVENT_Table, MAX_SEC_EVENT);

   if (min_FLAG  == RAISE)    sweepTable(MIN_EVENT_Table, MAX_MIN_EVENT);

   if (hr_FLAG   == RAISE)    sweepTable(HR_EVENT_Table, MAX_HR_EVENT);
